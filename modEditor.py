...
def create_tags(root):
	field_list = [
		...
		'Random', 'Unlock', 'OSSpecific',
		'Population',  'InHouse','Retention',
		'Iterative','NameGenerator'
		]
...
def populate_feature(feature):
	...
	field_list = [
		'Description', 'Unlock', 'DevTime', 
		'Innovation','Usability', 'Stability', 
		'CodeArt', 'Server', 'Dependencies'
		]
...
def add_dependencies_to_feature(dependencies, dependency_dict_list):
...
def set_field_text(parent, dictionary):
	for tag, line_edit in dictionary.items():
...
def add_category(self, Categories, dictionary, name):
	...
	if Categories is not None and name:
		...
		self.add_to_combobox(self.CATEGORYCOMBOBOX, Categories, 'd')
